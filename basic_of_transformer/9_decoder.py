...
class MultiheadAttentionHead(nn.Module):
    def __init__(self, token_embed_dim, head_dim, n_head, is_casual=False):
        super().__init__()
        self._n_head = n_head
    ...
    def forward(self, q, k, v):
        ...
        # compute attention
        attention = compute_attention(mh_q, mh_k, mh_v, is_casual=self._is_casual)
...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, n_head, ff_layer_dim, dropout):
        ...
        # dropout1/2 norm 1/2
        # ff layer
        self.self_attention = MultiheadAttentionHead(d_model, d_model, n_head, is_casual=False)
        self.cross_attention = MultiheadAttentionHead(d_model, d_model, n_head, is_casual=True)
        self.ff_layer = PreLayerNormFeedForward(d_model, ff_layer_dim, dropout)
    ...
    def forward(self, x, encoder_output):
        ...
        # norm > cross attention > dropout > residual >
        # ff layer

        # self attention
        normed_x = self.norm_1(x)
        attened_x = normed_x + self.dropout_1(
            self.self_attention(normed_x, normed_x, normed_x
        ))
        
        # cross attention
        normed_x = self.norm_2(attened_x)
        attened_x = normed_x + self.dropout_2(
            self.cross_attention(normed_x, encoder_output, encoder_output
        ))
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, x, encoder_output):
        output = x
        for clone in self.clones:
            output = clone(output, encoder_output)
        return output

num_head = 4
tf_encoder_layer = TransformerEncoderLayer(embed_dim, num_head, embed_dim*2, 0.5)
tf_decoder_layer = TransformerDecoderLayer(embed_dim, num_head, embed_dim*2, 0.5)

num_tf_layer = 5
tf_encoder = TransformerEncoder(tf_encoder_layer, num_tf_layer)
tf_decoder = TransformerDecoder(tf_decoder_layer, num_tf_layer)

tf_encoder_output = tf_encoder(tf_input)
tf_decoder_output = tf_decoder(tf_input, tf_encoder_output)

print(f"tf_input.shape: {tf_input.shape}")
print(f"tf_encoder_output.shape: {tf_encoder_output.shape}")
print(f"tf_decoder_output.shape: {tf_decoder_output.shape}")